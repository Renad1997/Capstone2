...
public class AddressController {
    ...
    @PostMapping("/add")
    public ResponseEntity addAddress(@Valid @RequestBody Address address, Errors errors ) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateAddress(@PathVariable Integer addressId,@Valid @RequestBody Address address , Errors errors ){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
